...
def fetch_analytical_sample() -> pd.DataFrame:
    ...
    with engine.connect() as conn:
        ...
        return pd.read_sql(text("SELECT unit_id, cycle, sensor_id, reading_value FROM fact_sensor_readings WHERE unit_id <= 15"), conn)
...
def clean_layout(fig):
    ...
    fig.update_yaxes(showgrid=True, gridcolor='#F3F4F6', zeroline=False, linecolor='#E5E7EB')
    return fig
...
def main():
    ...
    if not df_samp.empty:
        ...
        pivot = df_samp.pivot(index=['unit_id', 'cycle'], columns='sensor_id', values='reading_value').dropna(axis=1, how='all')
        ...
        # ANOMALY HEATMAP
        st.markdown("<h3>Emergence of Anomalies (Z-Score > 3)</h3>", unsafe_allow_html=True)
        st.markdown("""
        <div class="narrative-text">
        ...
        rolling_mean = pivot.groupby(level='unit_id').rolling(window=10, min_periods=1).mean().droplevel(0)
        rolling_std = pivot.groupby(level='unit_id').rolling(window=10, min_periods=1).std().droplevel(0)
        ...
        heatmap_data = anomalies.groupby(['unit_id', 'cycle_bucket'])['anomaly_count'].sum().reset_index()
        heatmap_pivot = heatmap_data.pivot(index='unit_id', columns='cycle_bucket', values='anomaly_count').fillna(0)
        ...
        fig_heatmap.update_layout(plot_bgcolor='white', paper_bgcolor='white', margin=dict(l=0, r=0, t=10, b=0))
        st.plotly_chart(fig_heatmap, use_container_width=True, config={'displayModeBar': False})

        # PIVOT TABLE: LIFESPAN VS EARLIEST ANOMALY
        st.markdown("<h3>Lifespan Quartile vs. Leading Indicator</h3>", unsafe_allow_html=True)
        st.markdown("""
        <div class="narrative-text">
        ...
        quartiles = pd.qcut(max_cycles, q=4, labels=['Q1 (Shortest)', 'Q2', 'Q3', 'Q4 (Longest)'])
        ...
        for unit in pivot.index.get_level_values('unit_id').unique():
            ...
            if not anom_rows.empty:
                ...
                first_anomalies.append({'unit_id': unit, 'earliest_anomaly_sensor': top_sensor, 'first_anomaly_cycle': first_cycle})
            else:
                first_anomalies.append({'unit_id': unit, 'earliest_anomaly_sensor': 'None', 'first_anomaly_cycle': None})
    ...
    if not df_rank.empty:
        ...
        with col_unit:
            ...
            default_idx = all_units.index(worst_unit_id_default) if worst_unit_id_default in all_units else 0
            selected_unit_id = st.selectbox("Select an Engine Unit to analyze:", all_units, index=default_idx)
        ...
        if not df_worst.empty:
            ...
            try:
                inflection_y = df_worst[df_worst['cycle'] >= inflection_cycle]['rolling_avg_7'].iloc[0]
            except:
            ...
            fig_trend.add_trace(go.Scatter(
                x=df_worst['cycle'], y=df_worst['reading_value'],
            ...
            # Rolling Avg
            fig_trend.add_trace(go.Scatter(
                x=df_worst['cycle'], y=df_worst['rolling_avg_7'],
                mode='lines', line=dict(color=ACCENT_COLOR, width=3, shape='spline'),
                name='7-Cycle Trend'
            ))
            ...
            if not df_valid_band.empty and not pd.isna(std_dev):
                fig_trend.add_trace(go.Scatter(
                    x=df_valid_band['cycle'].tolist() + df_valid_band['cycle'].tolist()[::-1],
                    y=(df_valid_band['rolling_avg_7'] + std_dev).tolist() + (df_valid_band['rolling_avg_7'] - std_dev).tolist()[::-1],
                    fill='toself',
            ...
            fig_trend.add_annotation(
                x=inflection_cycle, y=inflection_y,
                text=f"Maintenance Flag (~60 cycles remaining)",
                showarrow=True, arrowhead=2, arrowsize=1, arrowwidth=2,
                arrowcolor="#DC2626", ax=-80, ay=-40,
    ...
    failing_units = len(df_rul[df_rul['estimated_rul_cycles'] < 20]) if not df_rul.empty else 0
    ...
    with rcol1:
        if not df_rank.empty:
            df_plot_rank = df_rank.sort_values('degradation_slope', ascending=True).head(10)
            fig_rank = px.bar(
                df_plot_rank, 
                x='degradation_slope', 
                y='unit_id', 
            ...
            fig_rank.update_layout(coloraxis_showscale=False, yaxis_type='category', title_font=dict(size=14, color="#6B7280"))
    ...
    with rcol2:
        if not df_rul.empty:
            st.markdown("<div style='font-size: 14px; color: #6B7280; margin-bottom: 10px;'>Lowest Estimated RUL</div>", unsafe_allow_html=True)
            df_rul_sorted = df_rul.sort_values('estimated_rul_cycles', ascending=True).head(8)
            st.dataframe(
    ...
    latest_pass_rate = float(df_health['pass_rate_pct'].iloc[0]) if not df_health.empty else 0.0
    
    pcol1, pcol2 = st.columns(2)
    with pcol1:
        st.markdown(f"<div style='font-size:0.875rem; font-weight:600; color:#6B7280; margin-bottom:0.5rem'>DQ PASS RATE TREND (Latest: {latest_pass_rate}%)</div>", unsafe_allow_html=True)
        if not df_health.empty:
            fig_health = px.line(df_health, x='run_date', y='pass_rate_pct', markers=True)
    ...
    with pcol2:
        st.markdown("<div style='font-size:0.875rem; font-weight:600; color:#6B7280; margin-bottom:0.5rem'>SENSOR OUT OF RANGE ANOMALIES</div>", unsafe_allow_html=True)
        if not df_oor.empty:
            st.dataframe(
                ...
                column_config={
                    "sensor_name": "Sensor",
                    "recent_oor_pct": st.column_config.NumberColumn("Recent % OOR", format="%.1f%%"),
                    "all_time_oor_pct": st.column_config.NumberColumn("All-time % OOR", format="%.1f%%"),
                }
            )
    ...
    with st.expander("⚙️ Engineering Details & Pipeline Performance"):
        ...
        st.markdown("<div style='font-family: monospace; font-size: 0.85rem; font-weight: 600; margin-top: 1rem;'>EXPLAIN ANALYZE: v_degradation_rank</div>", unsafe_allow_html=True)
        explain_text = fetch_explain_analyze("SELECT * FROM v_degradation_rank LIMIT 100")
