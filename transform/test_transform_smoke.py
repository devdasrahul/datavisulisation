...
def make_synthetic_bronze(
    dest: Path, dataset: str, unit_id: int, n_cycles: int = 55
) -> Path:
    ...
    data: dict = {
        ...
        "op_setting_1": pl.Series(
            [RNG.uniform(0, 0.5) for _ in range(n)], dtype=pl.Float64
        ),
        "op_setting_2": pl.Series(
            [RNG.uniform(0, 0.5) for _ in range(n)], dtype=pl.Float64
        ),
    ...
    data["sensor_2"] = pl.Series(
        [RNG.uniform(610.0, 640.0) for _ in range(n)], dtype=pl.Float64
    )
    data["sensor_3"] = pl.Series(
        [RNG.uniform(1565.0, 1615.0) for _ in range(n)], dtype=pl.Float64
    )
    for i in range(4, 22):
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    assert (
        report.rows_in == 56
    ), f"Expected 56 rows in (55 + 1 dup), got {report.rows_in}"
    assert (
        report.rows_out == 55
    ), f"Expected 55 rows out (dup dropped), got {report.rows_out}"
    assert (
        report.duplicates_dropped == 1
    ), f"Expected 1 dup dropped, got {report.duplicates_dropped}"
    ...
    for meta_col in BRONZE_META_COLS:
        assert (
            meta_col not in silver_df.columns
        ), f"Bronze meta col {meta_col} not stripped"
    ...
    assert (
        len(anomaly_rows) > 0
    ), "No anomaly rows found (expected cycle=10 to be flagged)"

    print(
        f"  rows_in={report.rows_in}  rows_out={report.rows_out}  "
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    s1 = gold_df.filter((pl.col("unit_id") == 1) & (pl.col("sensor_id") == 1)).sort(
        "cycle"
    )
    assert (
        s1["rolling_avg_7"][0] is not None
    ), "rolling_avg_7 null at cycle 1 (min_periods=1 broken)"
    assert s1["rate_of_change"][0] is None, "rate_of_change should be null at cycle 1"
    assert (
        s1["rate_of_change"][1] is not None
    ), "rate_of_change should have a value at cycle 2"
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    # First run
    stats1 = run_transform(
        raw_dir, clean_dir, agg_dir, ["FD001"], force=False, dry_run=False
    )
    assert stats1.batches_transformed == 1
    assert stats1.batches_skipped == 0

    # Second run — should be a no-op
    stats2 = run_transform(
        raw_dir, clean_dir, agg_dir, ["FD001"], force=False, dry_run=False
    )
    assert stats2.batches_transformed == 0
    assert stats2.batches_skipped == 1
    print(
        f"  Run 1: transformed={stats1.batches_transformed}  skipped={stats1.batches_skipped}"
    )
    print(
        f"  Run 2: transformed={stats2.batches_transformed}  skipped={stats2.batches_skipped}"
    )
    print("  PASS")
